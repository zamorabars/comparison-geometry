...
Let us give an other formulation of Crofton formula.

Consider the space of oriented lines; 
an oriented line can be characterized by its direction --- a unit vector $w$ and the distance $\rho$ from the origin taken with sign --- it is positive if $w$ points counterclockwise, negative if clockwise.
The vector $w$ can be described by its angle $\phi$ to a fixed direction.
...
\begin{thm}{Crofton formula via intersections}
For any curve $\alpha$,
\[\length\alpha=\tfrac14\cdot\int_{(-\pi,\pi]\times \RR} n_\alpha(\rho,\phi)\cdot d\rho\cdot d\phi.\]
\end{thm}

In particular if $\alpha$ is rectifiable then most of lines intersect $\alpha$ at finite number of points.
...
Find a lower bound on the length of its trajectory (does not need to be exact).
